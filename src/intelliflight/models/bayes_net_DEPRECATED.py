...
# This file has comments where old code was removed from the actual model
...
class Bayes_Net(ai_model.AI_Model):
    def __init__(self, params_path: str = None):
        ...
        self.key_meta = modelutil.KeyMeta()
        # self.seen_airports: set = None
        # self.seen_carriers: set = None
        self.dataset = modelutil.Dataset()
        self.frequencies = modelutil.FrequencyCounter()
        self.p_tables = modelutil.ProbabilityTables()
    ...
    def train_model(self, partition_count: int, k_step_percent: float, max_k_fraction: float, rng_seed: int = None):
        ...
        for test_index in range(len(partitions)):
            ...
            for validation_index in range(len(partitions)):
                ...
                #     self.dataset.get_data(), data_len, test_start, test_end, validation_start, validation_end)
